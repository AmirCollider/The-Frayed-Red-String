...
15. [تاریخ و برگ‌ریزان](#۱۵-تاریخ-و-برگریزان)
...
| ۲۵ | `DateCard` | تاریخ را در گوشه نشان می‌دهد | ❌ |
| ۲۶ | `Fall` | چه چیزی می‌ریزد و چقدر — شکوفه، برگ، برف | ❌ |
...
| `Date` | `DateCard` | سه عدد. **روزِ هفته محاسبه می‌شود، هیچ‌وقت نوشته نمی‌شود** ← بخش ۱۵ |
| `Fall` · `FallDensity` | `Fall` | نوع و شدتِ ریزش. `Seconds` مدتِ تغییر است ← بخش ۱۵ |
...
| `Date(StoryDate)` · `Date(y, m, d)` | `DateCard` |
| `Fall(kind, density, seconds)` · `NoFall()` | `Fall` |
...
### جدا شدنِ کاراکتر از پس‌زمینه (Separation)

هر دو کاراکتر و همه‌ی اتاق‌ها در **یک کلیدِ رنگیِ پاستلی** کشیده شده‌اند. این خودِ سبک است و اشتباه نیست — ولی نتیجه‌اش این بود که ژاکتِ کرمِ هارو جلوی دیوارِ کرمِ کلاس تقریباً هیچ لبه‌ای نداشت و شانه‌اش در ساختمان حل می‌شد.

دو حرکتِ کوچک به‌جای یک حرکتِ بزرگ، چون هرکدام به‌تنهایی باید آن‌قدر زیاد شود که زشت شود:

| فیلد در `StageSettings.asset` | پیش‌فرض | چه می‌کند |
|---|---|---|
| `Separate From Scenery` | روشن | کلید هر دو حرکت. خاموش کنی، بازی دقیقاً مثلِ قبل می‌شود |
| `Scenery Recede` | `0.16` | پس‌زمینه چقدر عقب می‌رود. یک ضربِ ساده روی رنگِ لایه |
| `Scenery Recede Tint` | `(0.62, 0.66, 0.78)` | به کدام رنگ. سرد و کمی تیره — کاری که هوا با فاصله می‌کند. چون کاراکترها گرم‌اند، این آن‌ها را با **فام** هم جدا می‌کند نه فقط با روشنایی |
| `Character Edge` | `4.5` | بردِ کانتورِ نرمِ تیره‌ی دورِ کاراکتر، در واحدِ بوم (صفحه ۱۰۸۰ است). صفر یعنی بدونِ کانتور |
| `Character Edge Colour` | بنفشِ خیلی تیره، آلفا `0.34` | مشکیِ پررنگ شبیهِ استیکر می‌شود |

**کانتور با `UnityEngine.UI.Outline` است، نه با یک شیءِ دوم.** یعنی همان quad چهار بار در قطرها پشتِ خودش با یک رنگِ تخت کشیده می‌شود و چون آلفای خودِ اسپرایت را استفاده می‌کند، چیزی که می‌بینی **سیلوئتِ کاراکتر** است نه یک مستطیل. دو نتیجه دارد و هر دو را می‌خواستیم:

* با ورودِ کاراکتر محو می‌شود (`useGraphicAlpha`)، چون همان آلفا را می‌خواند که انیمیشنِ ورود روی آن کار می‌کند.
* هیچ `GameObject`، هیچ `AmbientMotion`ِ دوم و هیچ همگام‌سازی‌ای لازم ندارد. یک اسپرایتِ همراه پشتِ هر کاراکتر باید با حالت، اندازه، آینه‌شدن، ورود، کم‌نورشدنِ فوکوس و نفس‌کشیدن هماهنگ می‌ماند — در شش جا، برای همیشه.

**`Scenery Recede` فقط کانالِ رنگ را می‌نویسد، نه آلفا را.** کراس‌فِیدِ پس‌زمینه از `SetAlpha` رد می‌شود که به رنگ دست نمی‌زند، پس یک بار در `SetUpBackgrounds` اعمال می‌شود و برای همیشه می‌ماند.
...
## ۱۵. تاریخ و برگ‌ریزان

دو سیستمی که بعد از بازنویسیِ اسناد ساخته شدند. هر دو **اختیاری‌اند**: صحنه‌ای که از هیچ‌کدام استفاده نکند، دقیقاً مثلِ قبل از وجودشان اجرا می‌شود.

### ۱۵.۱ تاریخ

| چه چیزی | کجا |
|---|---|
| `StoryDate` · `DateStyle` · `StoryCalendar` | `_C#/Narrative/StoryCalendar.cs` |
| بیتش | `StoryBeatKind.DateCard` |
| فیلدهایش | `BeatData.Date` و `ActAsset.StartDate` |
| فعلش | `Date(...)` در `ActScriptWriter` |

```csharp
Date(StoryCalendar.TheFriendStory);   // تاریخِ قفل‌شده، از روی اسمش
Date(2024, 12, 18);                   // یا سه عدد
```

**قانونِ اولِ این سیستم:** روزِ هفته هیچ‌وقت تایپ نمی‌شود، محاسبه می‌شود. سند بازی را «دوشنبه ۱ سپتامبر ۲۰۲۴» شروع می‌کرد و آن روز **یکشنبه** است — و هیچ اجرایی از بازی هیچ‌وقت این را نمی‌گرفت. حالا `StoryCalendar.TryWeekdayOf` تنها منبعِ مجاز است، و پنجره‌ی Story Editor روزِ هفته را همان موقعِ تایپ کردنِ تاریخ زیرش می‌نویسد.

**قانونِ دوم:** `System.Globalization` استفاده نمی‌شود. `DateTime.ToString` کالچرِ دستگاهِ بازیکن را می‌خواند، پس یک بیلد روی لپ‌تاپِ ژاپنی و فارسی دو چیزِ متفاوت می‌نوشت — و زبانِ انتخابیِ بازیکن اصلاً در آن نقشی نداشت. جدولِ ماه‌ها و روزها داخلِ خودِ فایل است.

سه اجرا، نه سه ترجمه:

```
fa   دوشنبه ۲ سپتامبر ۲۰۲۴        ← ارقامِ فارسی
ja   2024年9月2日（月）
en   Monday, 2 September 2024
```

ماه‌ها **میلادی‌اند، نه شمسی** — داستان در ژاپن می‌گذرد و نیمی از تاریخ‌هایش سالگردِ همدیگرند؛ تبدیلِ تقویم برای یکی از سه زبان، همه‌ی آن قرینه‌ها را برای بازیکنانِ آن زبان می‌شکند.

**کجا دیده می‌شود:**

* **کارتِ عنوانِ پرده** — `ActAsset.StartDate`، با روزِ هفته، زیرِ خطِ جداکننده. خالی بگذاری، کارت دقیقاً همان است که همیشه بود.
* **بیتِ `DateCard`** — همان پلاکِ گوشه‌ای که اسمِ مکان از آن استفاده می‌کند، بدونِ روزِ هفته، حدودِ دو ثانیه. منتظرِ بازیکن نمی‌ماند.

**قانون‌های نوشتن:**

* تاریخ **فقط وقتی می‌آید که عوض شده باشد.** یکی در هر جهشِ زمانی، نه یکی سرِ هر صحنه. تاریخِ هر چند دقیقه یک بار، ساعت است؛ این بازی ساعت ندارد.
* **هیچ شخصیتی هیچ‌وقت تاریخ را بلند نمی‌گوید.** «امروز بیست‌ویکمِ دسامبره» دیالوگ نیست، اعلامِ وضعیت است — دستورنامه بخشِ ۹، الگوی ممنوعِ ۳.
* در حالتِ فیلم (پرده‌ی ششم) تاریخ فقط به‌عنوانِ یک برشِ عمدی می‌آید. آنجا تاریخ خودش افشاست: ۲۶ ژانویه ۲۰۱۷.

تاریخ‌های قفل‌شده به‌صورتِ ثابتِ نام‌دار در `StoryCalendar` هستند: `MachineRoom` · `ActOneDayOne` · `TheFriendStory` · `ThePencilCase` · `TheLastDay` و بقیه. **از اسم استفاده کن، نه از سه عدد.**

### ۱۵.۲ برگ‌ریزان

| چه چیزی | کجا |
|---|---|
| لایه و شکل‌هایش | `_C#/Presentation/SeasonalFall.cs` |
| بیتش | `StoryBeatKind.Fall` |
| فعلش | `Fall(kind, density, seconds)` و `NoFall()` |

```csharp
Fall(FallKind.MapleLeaf, 0.25f);      // یک بعدازظهرِ عادیِ پاییزی
Fall(FallKind.MapleLeaf, 0.6f, 4f);   // باد بلند می‌شود، در چهار ثانیه
NoFall();                             // هوا صاف می‌شود
```

پنج نوع: `None` · `Sakura` · `MapleLeaf` · `GinkgoLeaf` · `Snow`.

**با بیت کنترل می‌شود، نه با پس‌زمینه.** هوا در این بازی صحنه را دنبال می‌کند نه مکان را: باد وسطِ یک گفت‌وگو بلند می‌شود، و وقتی کسی شروع به حرف زدن با بازیکن می‌کند کاملاً می‌ایستد.

**قانونِ شکوفه:**

> بازی از سپتامبر تا مارس می‌گذرد. **هیچ‌وقت فصلِ شکوفه‌ی گیلاس نیست.**
> هر جا `FallKind.Sakura` بریزد، کسی دارد چیزی را می‌بیند که وجود ندارد — خاطره، رویا، یا سرابِ پرده‌ی اول.
> **هیچ‌کس هیچ‌وقت به آن اشاره نمی‌کند.**

پنجره‌ی Story Editor روی `Sakura` یک هشدارِ زرد می‌دهد. عمدی است.

**جزئیاتِ فنی:**

* روی `BackgroundCanvas` است، یعنی **پشتِ کاراکترها و پشتِ قاب** — برگ‌ها از جلوی صورتِ کسی رد نمی‌شوند و نوارهای مشکی آن‌ها را هم با بقیه می‌برند.
* اندازه‌ی لایه هر فریم از خودِ rect خوانده می‌شود، پس وقتی قاب در پرده‌ی پنجم باز می‌شود، ریزش هم با آن پهن می‌شود.
* روی `StoryClock` است، پس با منوی توقف می‌ایستد.
* موقعِ `EnterAside` خودکار خاموش می‌شود و با `ExitAside` برمی‌گردد. یک برگ که وسطِ آن دیالوگ رد شود، تنها چیزی روی صفحه است که می‌گوید این هنوز یک صحنه در یک بازی است.
* لودِ یک سیو وضعیتش را برمی‌گرداند (`PrimeStateUpTo`)، روی همان فریم و بدونِ فِید.
* هر پرده با هوای صاف شروع می‌شود.
* سقفش ۴۰ ذره است. `density = 1` یک روزِ بادی است، نه بوران.
* شکل‌ها با کد کشیده می‌شوند، مثلِ بقیه‌ی این پروژه. هیچ PNG ای لازم نیست.

**صدا:** `SfxId.LeafFall` کنارِ `SfxId.Petal` اضافه شد — خشک‌تر، بلندتر در فرکانس، کوتاه‌تر. برگِ مرده مثلِ گلبرگ صدا نمی‌دهد.

#### قانونِ «کجا مجاز است»

لایه‌ی ریزش **کلِ تصویر** را می‌پوشاند، نه پنجره‌ی داخلِ تصویر را. پس:

> **بیرون، و فقط در روشنایی.**
>
> * **داخلِ ساختمان هیچ‌وقت.** کلاس، راهرو، کافه، اتاق — همه `NoFall(0f)`. راهروی مدرسه پنجره‌ی سرتاسری دارد و افراها پشتش‌اند؛ آن‌ها **در خودِ نقاشی** هستند و لازم نیست چیزی روی صحنه بریزد.
> * **شب هیچ‌وقت.** دو صحنه‌ی شبِ پرده‌ی اول و صحنه‌ی شبِ پرده‌ی دوم همه `NoFall` هستند. برگ در تاریکی دیده نمی‌شود و فقط نویز است.
>
> هر دوی این‌ها یک بار شکسته شده بودند و هیچ‌کس ندید: کافه‌ی پرده‌ی اول روی `0.05` بود و **راهروی نوامبرِ پرده‌ی دوم روی `0.42`** — سنگین‌ترین ریزشِ کلِ بازی، زیرِ سقف.

#### باگی که این افکت را نامرئی کرده بود

خودِ لایه از روزِ اول درست کار می‌کرد. `VisualNovelStage.SwapBackgroundLayers()` بعد از هر تعویضِ صحنه، پس‌زمینه‌ی نمایان را با `SetAsLastSibling()` به **آخرِ همان parent** می‌فرستاد — و لایه‌ی ریزش هم بچه‌ی همان parent است. یعنی از **اولین `Place()` پرده‌ی اول** به بعد، برگ‌ها کشیده می‌شدند و بعد خودِ نقاشی رویشان می‌افتاد.

دو تعمیر، هر دو ماندگار:

* در `SwapBackgroundLayers()` دو لایه‌ی پس‌زمینه فقط **بینِ خودشان** جابه‌جا می‌شوند (`SetAsFirstSibling` و `SetSiblingIndex(1)`). هر چیزِ دیگری در آن parent بالای هر دو می‌ماند.
* `SeasonalFall.KeepOnTop()` هر فریم یک مقایسه‌ی عددی می‌کند و اگر کسی دوباره ترتیب را به هم زد، بی‌صدا برنمی‌گردد به همان باگ.

> **درسش:** `SetAsLastSibling()` روی یک parent مشترک یعنی «بالای هر چیزی که همسایه‌ام است»، نه «بالای همکارم». اگر منظورت دومی است، ایندکس بده.
