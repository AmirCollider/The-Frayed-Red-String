...
| ۲۵ | `Surprised` | `YuaSurprisedTakenAback` | `HaruSurprisedTakenAback` | دست روی سینه، چشم‌های گرد و دهان نیمه‌باز؛ وقتی طرف مقابل یک فکت پیش‌بینی‌نشده می‌آورد یا خلاف انتظار عمل می‌کند. |
| ۲۶ | `Bored` | `YuaBoredUnamused` | `HaruDeadpanFlat` | دست‌به‌سینه، چشم‌های بسته/بی‌تفاوت و لب‌های خط‌شده؛ واکنش پوکرفیس و بی‌حوصلگی کمدی به غرها، خیال‌بافی‌ها و رفتارهای عجیب طرف مقابل. |
...
| ۲۸ | `StrainedSmile` | — به `Sad/Neutral` می‌افتد — | `HaruStrainedPainfulSmile` | دست روی گردن/یقه، لبخند تلخ و زورکی؛ وقتی درد پایش را پنهان می‌کند یا سعی دارد وانمود کند همه‌چیز خوب است تا یوآ نگران نشود. |
| ۲۹ | `Sheepish` | — به `Shy` می‌افتد — | `HaruSheepishHandBehindHead` | دست پشتِ سر، سرخ شدن گونه‌ها و لبخند دستپاچه؛ وقتی یوآ مچش را می‌گیرد، به گیاه‌ها عذرخواهی می‌کند یا بهانه‌هایش لو می‌رود. |
| ۳۰ | `CalmSerious` | — به `Neutral` می‌افتد — | `HaruCalmSerious` | دست روی کمر، نگاه جدی و مطمئن؛ وقتی بحث‌های منطقی را می‌برد، واقعیت‌ها (جهت پنجره‌ها، نانوا، تاریخ‌ها) را می‌گوید یا نظراتش را با متانت بیان می‌کند. |
