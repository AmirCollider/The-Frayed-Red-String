...
namespace TheFrayedRedString.Narrative
{
    ...
    public sealed class StoryDirector : MonoBehaviour
    {
        ...
        /// <summary>
        /// Which option the player last took, so the loop knows which block of
        /// beats after a choice to play and which to step over.
        /// </summary>
        private int _takenChoice;
        ...
        /// <summary>
        /// True while a character has turned away from the scene and is
        /// speaking to the person holding the controller.
        /// </summary>
        /// <remarks>
        /// Held by the director rather than by the act, so it resets with every
        /// scene: an act that forgets to close an aside cannot leave the next
        /// one being played in a dark room.
        /// </remarks>
        private bool _aside;
        ...
        {
            // A save taken part-way down one road of a choice cannot simply be
            // resumed where it stopped: the beat that decides which road to walk
            // is behind us, so playing on from here would walk the rest of this
            // block and then every other block as well. Backing up to the choice
            // and asking again is the honest fix, and it costs the player
            // nothing — a branch is a handful of lines and the question it
            // answers has no consequence to lose.
            fromBeat = RewindOutOfBranch(fromBeat);
            ...
            for (int i = fromBeat; i < _act.Count; i++)
            {
                ...
                // A choice the player has just answered, with something behind
                // each option: play the block they chose, then put the cursor
                // past all of them so the story rejoins itself.
                int branched = TotalBranchLength(beat);
                if (branched > 0)
                {
                    yield return PlayBranch(beat, i);
        ...
        private IEnumerator PlayBeat(BeatData beat)
        {
            ...
            switch (beat.Kind)
            {
                ...
                case StoryBeatKind.EnterAside:
                    yield return SetAside(true);
                    break;

                case StoryBeatKind.ExitAside:
                    yield return SetAside(false);
                    break;
            }
        }

        /// <summary>
        /// Turns the room down behind somebody addressing the player, or gives
        /// it back.
        /// </summary>
        /// <remarks>
        /// Waited on rather than started and left behind. The withdrawal is the
        /// signal — a line arriving while the room is still on its way down
        /// reads as a transition somebody interrupted, and the whole point of
        /// these four scenes is that nothing is hurrying.
        /// </remarks>
        private IEnumerator SetAside(bool aside)
        {
            if (_aside == aside)
            {
                yield break;
        ...
        private IEnumerator PlayLine(BeatData beat)
        {
            ...
            // A line that asks for its own speed keeps it; everything else said
            // to the player comes slower than the game types anywhere else.
            float speed = beat.TypeSpeed > 0f
                ? beat.TypeSpeed
                : (_aside ? GameConfig.AsideTypeSpeed : 0f);

            Speak(beat.Speaker, beat.Text, beat.VoiceClip, speed);
        ...
        /// <summary>
        /// How many beats after a choice beat belong to its options in total.
        /// </summary>
        /// <remarks>
        /// Zero for every choice that changes nothing, which is what a choice
        /// written before branching existed reads as — so acts two through
        /// seven, the endings and every interlude go on behaving exactly as
        /// they did.
        /// </remarks>
        private static int TotalBranchLength(BeatData beat)
        {
            if (beat == null || beat.Kind != StoryBeatKind.Choice || beat.Choices == null)
            {
                return 0;
            }
            ...
            {
                total += Mathf.Max(0, beat.Choices[i].BranchLength);
            }
        ...
        /// <summary>
        /// Plays the block of beats behind the option the player took.
        /// </summary>
        /// <remarks>
        /// The blocks sit end to end after the choice in the order the options
        /// are listed, so the one to play starts past the lengths of all the
        /// options above it. Beats here still report themselves as the current
        /// beat, so the pause menu and a save taken mid-branch know where the
        /// player is — and <see cref="RewindOutOfBranch"/> knows what to do with
        /// that when the save is loaded again.
        /// </remarks>
        private IEnumerator PlayBranch(BeatData choice, int choiceBeat)
        {
            int taken = Mathf.Clamp(_takenChoice, 0, choice.Choices.Length - 1);

            int start = choiceBeat + 1;
            for (int i = 0; i < taken; i++)
            {
                start += Mathf.Max(0, choice.Choices[i].BranchLength);
            }

            int length = Mathf.Max(0, choice.Choices[taken].BranchLength);

            for (int i = start; i < start + length && i < _act.Count; i++)
            {
                CurrentBeat = i;
                StorySession.LineIndex = i;

                BeatData inner = _act.At(i);
        ...
        /// <summary>
        /// Moves a resume point that landed inside a choice's block back to the
        /// choice itself.
        /// </summary>
        /// <remarks>
        /// Returns the index unchanged when it is not inside one, which is every
        /// save in an act that has no branching choices in it.
        /// </remarks>
        private int RewindOutOfBranch(int beatIndex)
        {
            for (int i = 0; i < beatIndex && i < _act.Count; i++)
            {
                int total = TotalBranchLength(_act.At(i));

                if (total > 0 && beatIndex <= i + total)
                {
                    return i;
                }
            }
        ...
        private void PrimeStateUpTo(int index)
        {
            ...
            if (_grade != null)
            {
                ...
                _grade.SetAttention(aside ? GameConfig.AsideDim : 0f, 0f);
            }

            // A save taken in the middle of one of the four scenes where
            // somebody is talking to the player has to reopen in that scene, not
