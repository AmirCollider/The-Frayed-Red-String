...
namespace TheFrayedRedString.Narrative
{
    ...
    public static class StorySession
    {
        ...
        /// <summary>
        /// The slot this playthrough last came from or was written to, or 0 for
        /// a run that has never touched one.
        /// </summary>
        /// <remarks>
        /// Read by nothing at the moment, and that is deliberate. It used to be
        /// the target of an automatic write at every act boundary, which turned
        /// "the player once loaded this slot" into "the game may overwrite this
        /// slot without being asked". Saving is the player's decision here; this
        /// is a note about where the playthrough has been, not a permission.
        /// </remarks>
        ...
        /// <summary>
        /// Times the player has sat out one of the game's silences to the end.
        /// </summary>
        /// <remarks>
        /// The other half of the ending condition, and the half nobody is told
        /// about: the moments where Haru's leg stops him walking, and where the
        /// machine room reaches Yua through a floor, are the only two places
        /// this game asks for nothing and waits. A player who gives them
        /// <see cref="Core.GameConfig.PatienceSeconds"/> anyway is the player
        /// the closing card is addressed to.
        /// </remarks>
        public static int PatientMoments { get; private set; }

        /// <summary>True once the player has waited out at least one silence.</summary>
        public static bool HasBeenPatient => PatientMoments > 0;
        ...
        /// <summary>True once the player has taken any option that counts.</summary>
        /// <remarks>
        /// White options are not counted and never were. Before the first blue
        /// or green press the player has committed to nothing, and neither of
        /// the two conditions below is satisfiable — which matters, because act
        /// two puts both of its silences before its first choice.
        /// </remarks>
        public static bool HasChosen => KindChoices > 0 || CruelChoices > 0;

        /// <summary>
        /// True when the player has taken the kind option and never once the
        /// controlling one.
        /// </summary>
        /// <remarks>
        /// The condition for the secret ending. A single green press anywhere in
        /// the game closes it off permanently, and no amount of blue afterwards
        /// reopens it.
        /// </remarks>
        public static bool IsPureKindRun => HasChosen && CruelChoices == 0;

        /// <summary>
        /// True when the player has taken the controlling option and never once
        /// the kind one.
        /// </summary>
        /// <remarks>
        /// The condition for the open ending, and the mirror of the one above. A
        /// single blue press closes this one off just as permanently — which is
        /// the point of the pair: both endings are about having meant something
        /// consistently, and neither is a reward for having tried a bit of each.
        /// </remarks>
        public static bool IsPureCruelRun => HasChosen && KindChoices == 0;

        /// <summary>
        /// True when the player has pressed both colours at least once.
        /// </summary>
        /// <remarks>
        /// <para>
        /// The most common way to play, and the one that earns nothing. Sitting
        /// through five minutes of a silence on a mixed run raises no offer at
        /// all: waiting is not the condition, it is what the condition is
        /// redeemed with, and there is nothing to redeem.
        /// </para>
        /// <para>
        /// Nothing tells the player this, before or afterwards. The game does
        /// not say what it is counting and does not say when it has stopped
        /// mattering.
        /// </para>
        /// </remarks>
        public static bool HasMixedChoices => KindChoices > 0 && CruelChoices > 0;
        ...
        public static void Resume(SaveSlotData slot)
        {
            ...
            PatientMoments = Mathf.Max(0, slot.PatientMoments);
        ...
        /// <summary>
        /// Records that the player sat with one of the game's silences long
        /// enough for it to count.
        /// </summary>
        public static void RecordPatience()
        {
            PatientMoments++;
        }
...
#if UNITY_EDITOR
        /// <summary>
        /// Sets the two choice counters directly, for testing the endings.
        /// </summary>
        /// <remarks>
        /// Editor only, and compiled out of a build entirely. Reaching a
        /// qualifying run honestly means pressing the same colour at every
        /// choice from act two onward without slipping once — twenty minutes of
        /// careful play before the branch being tested is even reachable. See
        /// the Endings menu.
        /// </remarks>
        public static void OverrideChoiceCounts(int kind, int cruel)
        {
            KindChoices = Mathf.Max(0, kind);
            CruelChoices = Mathf.Max(0, cruel);
        }
#endif
