...
namespace TheFrayedRedString.Narrative
{
    ...
    public struct ChoiceData
    {
        ...
        /// <summary>
        /// How many of the beats after the choice belong to this option.
        /// </summary>
        /// <remarks>
        /// <para>
        /// Zero — the default, and what every act written before this field
        /// existed has — means the choice changes nothing and the story carries
        /// straight on. That was every choice in the game, and it was the whole
        /// problem: a button the player pressed and the game did not answer.
        /// </para>
        /// <para>
        /// When it is set, the options' blocks sit end to end immediately after
        /// the choice beat, in the order the options are listed. Picking option
        /// <c>k</c> plays block <c>k</c> and skips the rest, and the story
        /// rejoins itself at the first beat after the last block — so the two
        /// roads differ for a while and then converge, which is the only shape
        /// that does not double the length of the act with every button.
        /// </para>
        /// <para>
        /// A block may hold <see cref="StoryBeatKind.Line"/> beats and nothing
        /// else, and that restriction is doing real work: the stage, the
        /// background, the music and the frame are all rebuilt from scratch when
        /// a save is loaded, by replaying every stage instruction before the
        /// resume point at zero duration. A block that moved somebody or changed
        /// the picture would be replayed whether it was taken or not, and a save
        /// made after a choice would come back with the other road's stage. Lines
        /// are skipped by that replay, so a block of them cannot lie about
        /// anything. The Story Editor's Check tab reports any block that holds
        /// something else.
        /// </para>
        /// </remarks>
        [Tooltip("How many of the following beats belong to this option. 0 = the choice changes nothing.")]
