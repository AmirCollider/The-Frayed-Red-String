...
namespace TheFrayedRedString.Scenes
{
    ...
    public sealed class ActSceneController : MonoBehaviour
    {
        ...
        private void Start()
        {
            ...
            RectTransform storyLayer = CreateCanvas("StoryCanvas", GameConfig.StoryCanvasOrder);

            BuildStage(authoredBackground, backgroundLayer, storyLayer);
            BuildDialogue(storyLayer);
            BuildFrame(storyLayer);
            BuildPauseLayer();
            BuildDirector();
        ...
        private void BuildDialogue(RectTransform storyLayer)
        {
            // Order is set by creation order: the click catcher goes in above the
            // characters and below everything the player can read, so a press
            // anywhere that is not a button advances the story.
            BuildAdvanceCatcher(storyLayer);
            ...
            _dialogue.Initialize(storyLayer);
        ...
        /// </remarks>
        private void BuildFrame(RectTransform storyLayer)
        {
        ...
        private void AdoptOldMenuCanvas(RectTransform layer)
        {
            ...
            {
                languageMask.SetParent(layer, false);
                languageMask.SetAsLastSibling();
