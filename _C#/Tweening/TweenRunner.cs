...
namespace TheFrayedRedString.Tweening
{
    ...
    public sealed class TweenRunner : MonoBehaviour
    {
        ...
        /// <param name="pausesWithStory">
        /// Run on <see cref="StoryClock"/> rather than on the wall clock, so the
        /// animation stops dead while the pause menu is up. For anything that
        /// moves the picture; never for the fade curtain or the menu itself,
        /// which have to keep working while the game is held.
        /// </param>
            ...
            bool pausesWithStory = false)
        {
            ...
            Coroutine routine = runner.StartCoroutine(
                Step(handle, duration, onUpdate, ease, delay, onComplete, owner, useUnscaledTime, pausesWithStory));
