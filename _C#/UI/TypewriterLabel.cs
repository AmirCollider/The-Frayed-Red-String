...
namespace TheFrayedRedString.UI
{
    ...
    public sealed class TypewriterLabel : MonoBehaviour
    {
        ...
        public void Play(string logical, Speaker speaker, float charactersPerSecond = 0f)
        {
            if (_label == null)
            {
                return;
            ...
            // Zero means "whatever this label was built with", so a line that
            // does not ask for a speed is unaffected and one that does is not
            // permanent.
            _charactersPerSecond = charactersPerSecond > 0f
                ? charactersPerSecond
                : GameConfig.TypeSpeedNormal;
        ...
        private static SfxId VoiceFor(Speaker speaker)
        {
            switch (speaker)
            {
                ...
                case Speaker.YuaChild:
                    return SfxId.TypeYua;

                case Speaker.Haru:
                case Speaker.HaruChild:
                    return SfxId.TypeHaru;

                default:
                    return SfxId.TypeNarrator;
            }
        }
