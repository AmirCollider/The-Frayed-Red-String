...
namespace TheFrayedRedString.UI
{
    ...
    public sealed class TypewriterLabel : MonoBehaviour
    {
        ...
        /// <summary>Multiplier applied to the reveal rate. Raised while fast-forwarding.</summary>
        ...
        private void Update()
        {
            ...
            float rate = Mathf.Max(1f, _charactersPerSecond * Mathf.Max(0.1f, SpeedMultiplier));
            _characterBudget += Time.unscaledDeltaTime * rate;

            // Punctuation debt is paid in the same currency as characters, so a
            // pause automatically scales with the reveal speed and disappears
            // while the player is holding fast-forward.
