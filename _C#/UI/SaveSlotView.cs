...
namespace TheFrayedRedString.UI
{
    ...
    {
        /// <summary>
        /// The thumbnail's tint on a slot that has never been written.
        /// </summary>
        /// <remarks>
        /// The grey the card was authored with. An empty slot keeps it; a
        /// written one goes to white, which is what lets the picture underneath
        /// actually show and what makes "this one has something in it" readable
        /// across the panel at a glance.
        /// </remarks>
        private static readonly Color EmptyTint = new Color32(0x4C, 0x4C, 0x4C, 0xFF);
        ...
        private Sprite _authoredThumbnail;
        private Image.Type _authoredThumbnailType;
        private StageSpriteLibrary _backgrounds;

        /// <summary>Which slot this card shows, 1-based.</summary>
        public int SlotNumber => _slotNumber;

        /// <summary>
        /// Raised after this card has been written, with its slot number.
        /// </summary>
        /// <remarks>
        /// The panel closes on this. Leaving it open after a write is what
        /// turned one deliberate save into two accidental ones: the cards stay
        /// on screen, one of them visibly changes, and the natural next thing a
        /// player does is press another card to see what is in it — which, in
        /// save mode, overwrites it.
        /// </remarks>
        public event System.Action<int> Written;

        /// <summary>Whether pressing this card reads the slot or writes it.</summary>
        public LoadPanelMode Mode { get; set; } = LoadPanelMode.Load;
        ...
        public void Refresh()
        {
            ...
            if (!_data.Exists)
            {
                StoryText.Set(_emptyLabel, LocalizationService.Get(LocKeys.LoadSlotEmpty));
                return;
            }

            StoryText.Set(
                _seasonLabel,
                LocalizationService.Format(LocKeys.LoadSlotSeason, _data.ActNumber));

            StoryText.Set(_seasonNameLabel, _data.GetActTitle());

            StoryText.Set(
                _playTimeLabel,
                LocalizationService.Format(LocKeys.LoadSlotPlayTime, _data.GetFormattedPlayTime()));
        }

        /// <summary>
        /// Shows the place the save was taken, or the empty card.
        /// </summary>
        /// <remarks>
        /// The background rather than a screenshot of the screen. It is the
        /// thing the player actually remembers about a moment — the café, the
        /// rooftop, the alley in the rain — and it costs one string in the save
        /// file instead of a megabyte of PNG per slot written to disk and read
        /// back on every panel open.
        /// </remarks>
        private void ApplyThumbnail()
        {
            ...
            if (_data.Exists && !string.IsNullOrEmpty(_data.BackgroundName))
            {
                if (_backgrounds == null)
                {
                    _backgrounds = StageSpriteLibrary.Load();
                }

                background = _backgrounds != null
                    ? _backgrounds.FindBackground(_data.BackgroundName)
                    : null;
            }

            if (background != null)
            {
                _screenshot.sprite = background;
                _screenshot.type = Image.Type.Simple;
                _screenshot.color = Color.white;
                return;
            }

            // No picture to show: either the slot is empty, or it holds a save
            // written before the background was recorded. Both get the card the
            // scene was authored with.
            _screenshot.sprite = _authoredThumbnail;
            _screenshot.type = _authoredThumbnailType;
            _screenshot.color = _data.Exists ? Color.white : EmptyTint;
        }
        ...
        private void ResolveLabels()
        {
            ...
            if (_screenshot != null)
            {
                // Captured once, before anything has been swapped in, so an
                // empty slot can be put back exactly as it was drawn.
                _authoredThumbnail = _screenshot.sprite;
                _authoredThumbnailType = _screenshot.type;
            }
        ...
        {
            if (Mode == LoadPanelMode.Save)
            {
                // An empty slot is the *best* target when saving, so the
                // "nothing here" refusal that guards loading must not fire.
                StorySession.WriteTo(_slotNumber);
                AudioService.Play(SfxId.Confirm, 0.85f);

                Written?.Invoke(_slotNumber);
                return;
            }
