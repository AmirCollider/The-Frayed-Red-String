...
namespace TheFrayedRedString.UI
{
    ...
    public sealed class DialogueBoxView : MonoBehaviour
    {
        ...
        /// <summary>
        /// The children's plates: their own colours, drained.
        /// </summary>
        /// <remarks>
        /// Recognisably the same person and visibly not the same moment. Paler
        /// rather than a different hue on purpose — a new colour would read as a
        /// new character, and this is the same one from further away.
        /// </remarks>
        private static readonly Color YuaChildPlate = new Color(0.93f, 0.74f, 0.82f, 0.94f);

        private static readonly Color HaruChildPlate = new Color(0.71f, 0.82f, 0.94f, 0.94f);

        /// <summary>
        /// The classmate's plate: the one colour in act one that is neither of
        /// theirs.
        /// </summary>
        /// <remarks>
        /// A pale green, chosen because it is the only hue the game has not
        /// spent — pink is Yua, blue is Haru, violet is the player, and the
        /// green of the cruel button does not appear until act two. Somebody
        /// from outside the pair should not arrive wearing either of their
        /// colours.
        /// </remarks>
        private static readonly Color ClassmatePlate = new Color(0.64f, 0.84f, 0.70f, 0.96f);
        ...
        /// <summary>The scrim while somebody is speaking to the player.</summary>
        private static readonly Color AsideScrimColour = new Color(0.06f, 0.02f, 0.05f, 0.78f);

        /// <summary>True while a character is addressing the player.</summary>
        private bool _aside;
        ...
        private static Color PlateColour(Speaker speaker)
        {
            switch (speaker)
            {
                ...
                case Speaker.YuaChild: return YuaChildPlate;
                case Speaker.HaruChild: return HaruChildPlate;
                case Speaker.Classmate: return ClassmatePlate;
        ...
        /// <summary>
        /// Puts the box on a heavier ground while a character is speaking to the
        /// player.
        /// </summary>
        /// <remarks>
        /// Only the scrim, and only when the panel is still there — once act
        /// five has taken the box away the lines are already on the heavy ground
        /// and there is nothing to deepen.
        /// </remarks>
        public void SetAsideMode(bool aside)
        {
            if (_aside == aside || _bare)
            {
                return;
            }
            ...
            if (_scrimImage != null)
            {
                _scrimImage.sprite = ProceduralUiSprites.VerticalGradient(
                    ScrimTop, aside ? AsideScrimColour : ScrimBottom);
            }
        }
