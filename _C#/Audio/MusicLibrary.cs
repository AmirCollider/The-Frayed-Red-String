...
namespace TheFrayedRedString.Audio
{
    ...
    public static class MusicTracks
    {
        ...
        /// </summary>
        /// <remarks>
        /// The file does not exist yet. Nothing breaks in the meantime: asking
        /// for a missing track logs one warning and the act plays silent, so the
        /// name can be reserved here and the mp3 dropped into
        /// <c>Assets/Audio/BackgroundMusics</c> whenever it is ready — the
        /// library rebuilds itself on import and the track starts working with
        /// no code change. Spelled to match the existing file's "BackGrung".
        /// </remarks>
        public const string ActOne = "Act01BackGrungMusic";

        /// <summary>
        /// The loop for an act, by 1-based number, following the same naming
        /// pattern so later acts need no new constants.
        /// </summary>
        public static string ForAct(int actNumber)
        {
            return $"Act{actNumber:00}BackGrungMusic";
        }
    }
