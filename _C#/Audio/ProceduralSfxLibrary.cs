    ...
    public static class ProceduralSfxLibrary
    {
        ...
        private static AudioClip Build(SfxId id)
        {
            switch (id)
            {
                ...
                // The frame giving way. A hard fracture and a scatter of high
                // inharmonic grains falling away from it — pitched nowhere near
                // the pentatonic everything else in this palette is tuned to,
                // because this is the sound of that palette ending.
                case SfxId.GlassBreak:
                    return ProceduralAudioSynth.Render(
                        ...
                        voices: new[]
                        {
                            Note(2100f, 0.000f, 0.30f, 0.42f, Timbre.Bell, decay: 22f, noise: 0.55f),
                            Note(1487f, 0.010f, 0.45f, 0.30f, Timbre.Bell, decay: 16f, pan: -0.30f),
                            Note(3160f, 0.018f, 0.28f, 0.22f, Timbre.Bell, decay: 26f, pan: 0.34f),

                            // The body of the pane, under the crack.
                            Note(180f, 0.000f, 0.55f, 0.34f, Timbre.Wood, decay: 11f, noise: 0.40f)
                        },
                        sparkles: new[]
                        {
                            Sparkle(34, 0.06f, 0.85f, 1900f, 3, 0.075f, 0.95f, 0.10f)
                        },
                        reverbMix: 0.40f,
                        reverbSeconds: 2.0f,
                        seed: 5051);

                // The gunshot. No pitch anybody could name, a very fast decay,
                // and a tail long enough to put it in a corridor.
                case SfxId.Gunshot:
                    return ProceduralAudioSynth.Render(
                        "SFX_Gunshot",
                        totalDuration: 3.20f,
                        voices: new[]
                        {
                            // The crack. Almost pure noise: the amplitude is
                            // carried by the transient, not by the note under it.
                            Note(140f, 0.000f, 0.22f, 0.85f, Timbre.Soft, decay: 42f, noise: 1.00f),
                            Note(62f, 0.000f, 0.60f, 0.70f, Timbre.Soft, decay: 9f, noise: 0.55f),

                            // The slap coming back off the far wall, a few
                            // milliseconds late and a little off to one side.
                            Note(96f, 0.035f, 0.45f, 0.30f, Timbre.Wood, decay: 14f, noise: 0.70f, pan: -0.45f),
                            Note(88f, 0.062f, 0.40f, 0.20f, Timbre.Wood, decay: 15f, noise: 0.65f, pan: 0.50f)
                        },
                        sparkles: null,
                        reverbMix: 0.62f,
                        reverbSeconds: 2.8f,
                ...
                //  Act one's room tone
                //
                //  The production book asks for these as build instructions
                //  rather than file names, and gives a target for each. What it
                //  specifies — a swept bandpass, an FM pair, brown noise — this
                //  synth does not have; it offers struck partials with a noise
                //  fraction, a decay and a reverb, which is what all twenty-three
                //  sounds above are made of. So each of these is that target
                //  approached with the vocabulary that exists, and where the
                //  method differs from the book the intent does not: the numbers
                //  below are the book's frequencies and durations.
                // -------------------------------------------------------------

                // Book: noise through a bandpass swept 800→300 Hz over 0.4 s.
                // Here: four noise-heavy partials walking down that same span on
                // staggered starts, which is a sweep made of steps — at 40 ms a
                // step nobody hears the stairs.
                case SfxId.ChairScrape:
                    return ProceduralAudioSynth.Render(
                        "SFX_ChairScrape",
                        totalDuration: 0.52f,
                        voices: new[]
                        {
                            Note(800f, 0.00f, 0.16f, 0.20f, Timbre.Soft, decay: 5.0f, noise: 0.94f, pan: -0.10f),
                            Note(620f, 0.04f, 0.16f, 0.22f, Timbre.Soft, decay: 5.0f, noise: 0.95f),
                            Note(450f, 0.09f, 0.16f, 0.20f, Timbre.Soft, decay: 5.5f, noise: 0.96f, pan: 0.08f),
                            Note(300f, 0.14f, 0.18f, 0.16f, Timbre.Soft, decay: 6.0f, noise: 0.97f),

                            // The leg finally leaving the floor.
                            Note(180f, 0.26f, 0.10f, 0.10f, Timbre.Wood, decay: 12f, noise: 0.55f)
                        },
                        sparkles: null,
                        reverbMix: 0.20f,
                        reverbSeconds: 0.7f,
                ...
                // Book: 90 Hz sine with a noise click, decay 0.12 s.
                case SfxId.DeskKnock:
                    return ProceduralAudioSynth.Render(
                        "SFX_DeskKnock",
                        totalDuration: 0.42f,
                        voices: new[]
                        {
                            Note(90f, 0f, 0.22f, 0.62f, Timbre.Wood, decay: 8.3f, noise: 0.10f),

                            // The board of the desk over the thump of it.
                            Note(226f, 0f, 0.12f, 0.24f, Timbre.Wood, decay: 14f, noise: 0.42f, pan: 0.12f),
                            Note(1450f, 0f, 0.04f, 0.13f, Timbre.Soft, decay: 30f, noise: 0.92f)
                        },
                        sparkles: null,
                        reverbMix: 0.16f,
                        reverbSeconds: 0.5f,
                ...
                // Book: FM, carrier 880 Hz, modulator ratio 1.4, index 3→0 over
                // 2.5 s. Without an FM operator the audible result of that index
                // sweep is a bright inharmonic partial at 1.4x that dies faster
                // than the fundamental, so that is written directly.
                case SfxId.SchoolBell:
                    return ProceduralAudioSynth.Render(
                        "SFX_SchoolBell",
                        totalDuration: 2.60f,
                        voices: new[]
                        {
                            Note(880f, 0f, 2.40f, 0.34f, Timbre.Bell, decay: 1.30f, noise: 0.02f),
                            Note(1232f, 0f, 1.10f, 0.20f, Timbre.Bell, decay: 3.40f, noise: 0.03f, pan: 0.16f),
                            Note(1760f, 0f, 0.50f, 0.09f, Timbre.Bell, decay: 6.00f, noise: 0.04f, pan: -0.14f),

                            // The second strike of a two-tone bell, a beat later.
                            Note(660f, 0.62f, 1.80f, 0.26f, Timbre.Bell, decay: 1.50f, noise: 0.02f, pan: -0.10f)
                        },
                        sparkles: null,
                        reverbMix: 0.44f,
                        reverbSeconds: 2.2f,
                ...
                // Book: 30 ms noise burst plus a 70 Hz thump decaying over 0.2 s.
                case SfxId.VendingThunk:
                    return ProceduralAudioSynth.Render(
                        "SFX_VendingThunk",
                        totalDuration: 0.60f,
                        voices: new[]
                        {
                            // The coin going in.
                            Note(2100f, 0f, 0.03f, 0.20f, Timbre.Soft, decay: 40f, noise: 0.96f),
                            Note(1400f, 0.01f, 0.05f, 0.14f, Timbre.Bell, decay: 26f, noise: 0.60f, pan: 0.2f),

                            // The mechanism, and then nothing.
                            Note(70f, 0.04f, 0.26f, 0.50f, Timbre.Wood, decay: 5.0f, noise: 0.14f),
                            Note(140f, 0.05f, 0.16f, 0.16f, Timbre.Soft, decay: 9.0f, noise: 0.30f, pan: -0.15f)
                        },
                        sparkles: null,
                        reverbMix: 0.22f,
                        reverbSeconds: 0.9f,
                ...
                // Book: two noise bursts 90 ms apart, with a sine under them.
                case SfxId.CanDrop:
                    return ProceduralAudioSynth.Render(
                        "SFX_CanDrop",
                        totalDuration: 0.85f,
                        ...
                        {
                            // The drop.
                            Note(1800f, 0f, 0.05f, 0.24f, Timbre.Soft, decay: 28f, noise: 0.93f),
                            Note(150f, 0f, 0.18f, 0.34f, Timbre.Wood, decay: 7.0f, noise: 0.20f),

                            // The settle, ninety milliseconds later.
                            Note(1600f, 0.09f, 0.06f, 0.17f, Timbre.Soft, decay: 24f, noise: 0.94f, pan: 0.18f),
                            Note(190f, 0.09f, 0.22f, 0.22f, Timbre.Wood, decay: 8.5f, noise: 0.24f, pan: 0.10f),

                            // Aluminium ringing on afterwards, faintly.
                            Note(3200f, 0.10f, 0.34f, 0.05f, Timbre.Bell, decay: 8.0f, noise: 0.12f, pan: -0.2f)
                        },
                        sparkles: null,
                        reverbMix: 0.30f,
                        reverbSeconds: 1.3f,
