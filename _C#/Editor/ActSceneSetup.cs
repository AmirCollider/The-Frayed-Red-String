...
namespace TheFrayedRedString.EditorTools
{
    ...
    public static class ActSceneSetup
    {
        ...
        /// onto the anchor and set back to a scale of one, and then measured —
        /// which writes the anchor into the settings together with the sprite's
        /// real height, so the two can no longer drift apart. A scene with no
        /// markers just gets the built-in numbers written into the asset.
        /// </para>
        /// </remarks>
        ...
        public static void ResetCharacterPlacement()
        {
            ...
            if (adopted > 0)
            {
                Debug.Log(
                    ...
                    $"(Yua x {StageSettings.Placement.YuaAnchorX}, Haru x {StageSettings.Placement.HaruAnchorX}, " +
            ...
            Debug.Log(
                markers > 0
                    ...
                    : $"[Stage] Both characters are back on the anchors: Yua x {StageSettings.Placement.YuaAnchorX}, " +
                      $"Haru x {StageSettings.Placement.HaruAnchorX}, both y {StageSettings.Placement.AnchorY}. " +
                      "The open scene has no character markers, so every act now reads these numbers.");
        }

        /// <summary>
        /// Moves one character's marker sprite onto its anchor.
        /// </summary>
        /// <returns>False when this scene has no marker for that character.</returns>
        /// <remarks>
        /// The scale is put back to one along with the position. The eight
        /// centimetres between Haru and Yua are baked into the artwork — Yua's
        /// canvas carries the top margin — so a marker that has been scaled by
        /// hand is a marker that has had that correction applied twice.
        /// </remarks>
