...
namespace TheFrayedRedString.EditorTools
{
    ...
    public sealed class StoryEditorWindow : EditorWindow
    {
        ...
        private List<Finding> Check()
        {
            ...
            if (_act.ActNumber <= 0 && !_act.name.StartsWith("Interlude", System.StringComparison.OrdinalIgnoreCase))
            {
                findings.Add(new Finding(
                    MessageType.Warning,
                    "This act has no number, so no scene will ever play it. Numbered acts are played by the " +
                    "scene of the same number; only interludes are meant to have none."));
            }
        ...
        private void DrawStageTab()
        {
            ...
            using (new EditorGUILayout.HorizontalScope())
            {
                ...
                if (GUILayout.Button(
                        new GUIContent("Reset to built-in", "Put both characters back where the code says."),
                        GUILayout.Width(130f),
                        GUILayout.Height(24f)))
                {
                    _settings.SetPlacement(StageSettings.Placement.Default(Speaker.Yua));
                    _settings.SetPlacement(StageSettings.Placement.Default(Speaker.Haru));
