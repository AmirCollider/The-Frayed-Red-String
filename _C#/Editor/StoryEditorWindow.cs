...
namespace TheFrayedRedString.EditorTools
{
    ...
    public sealed class StoryEditorWindow : EditorWindow
    {
        ...
        private static readonly StoryBeatKind[] QuickAddPresentation =
        {
            ...
            StoryBeatKind.Video,
            StoryBeatKind.EnterAside,
            StoryBeatKind.ExitAside,
        ...
        /// </summary>
        /// <remarks>
        /// Written through <c>intValue</c> rather than <c>enumValueIndex</c>
        /// throughout this window. The index is a position in the enum's list of
        /// names; the value is the number the asset stores. They agree only
        /// while an enum happens to run 0, 1, 2 with nothing skipped — and
        /// StoryBeatKind carries explicit numbers precisely so that a new kind
        /// can be added without renumbering every act already written, which is
        /// exactly the case where the two stop agreeing.
        /// </remarks>
        /// <remarks>
        ...
        {
            beat.FindPropertyRelative("Kind").intValue = (int)kind;
            ...
            for (int i = 0; i < 2; i++)
            {
                ...
                choice.FindPropertyRelative("Tone").intValue = i == 0 ? (int)ChoiceTone.Kind : (int)ChoiceTone.Cruel;
            }
        }
        ...
        private void BuildList()
        {
            ...
            _list.drawElementCallback = (rect, index, active, focused) =>
            {
                ...
                StoryBeatKind kind = (StoryBeatKind)beat.FindPropertyRelative("Kind").intValue;
        ...
        private string SummaryOf(SerializedProperty beat, StoryBeatKind kind)
        {
            switch (kind)
            {
                case StoryBeatKind.Line:
                {
                    Speaker speaker = (Speaker)beat.FindPropertyRelative("Speaker").intValue;
                ...
                case StoryBeatKind.Enter:
                    return $"→ {(Speaker)beat.FindPropertyRelative("Speaker").intValue} enters " +
                           $"({(Portrait)beat.FindPropertyRelative("Portrait").intValue})";

                case StoryBeatKind.Exit:
                    return $"← {(Speaker)beat.FindPropertyRelative("Speaker").intValue} leaves";
                ...
                case StoryBeatKind.Sound:
                    return "♪ " + (SfxId)beat.FindPropertyRelative("Sound").intValue;
                ...
                case StoryBeatKind.EnterAside:
                    return "◉ they turn and speak to the player";
        ...
        private static string DescriptionOf(StoryBeatKind kind)
        {
            switch (kind)
            {
                ...
                case StoryBeatKind.EnterAside:
                    return "The character stops talking to the scene and talks to the player. The room "
                           + "dims away behind them, the lines come slower than anywhere else in the game, "
                           + "and the box sits on a heavier ground. Close it with an Exit Aside.";

                case StoryBeatKind.ExitAside:
                    return "Give the scene back to the room. An act that leaves an aside open plays its "
                           + "next scene in the dark.";
        ...
        private void DrawBeatDetail()
        {
            using (new EditorGUILayout.VerticalScope(EditorStyles.helpBox))
            {
                ...
                StoryBeatKind kind = (StoryBeatKind)kindProp.intValue;
        ...
        /// <summary>
        /// The recipe for a silence, on the beat that is one.
        /// </summary>
        /// <remarks>
        /// <para>
        /// A silence is two beats, not one, and only one of them is ticked. The
        /// tick makes the wait count; the line before it — somebody speaking in
        /// the nine-year-old's voice — is the only thing that tells the player
        /// this moment is worth staying at. Written apart, they are a convention
        /// somebody has to have been told about, which is how the first seven
        /// got written and then very nearly shipped without their halves.
        /// </para>
        /// <para>
        /// So the window checks for the other half and offers to write it. The
        /// button is the documentation.
        /// </para>
        /// </remarks>
        private void DrawSilenceRecipe()
        {
            Speaker child = ChildVoiceAbove(_list.index);

            if (child != Speaker.Narrator)
            {
                EditorGUILayout.HelpBox(
                    $"This silence has its tell: {child} speaks a few beats above it, so the player sees " +
                    "a name plate reading 'aged nine' and knows, without being told, that this moment is " +
                    "different. Five minutes here counts once towards the endings.",
                    MessageType.Info);
            ...
            EditorGUILayout.HelpBox(
                "Five minutes on this line counts once towards the endings — and nothing on screen says " +
                "so, which means as written nobody will ever find it.\n\n" +
                "Every other silence in the game is two beats. A beat or two above it, one of them speaks " +
                "in their nine-year-old voice: the name plate reads 'aged nine', the line is a fragment " +
                "nobody asked for, and nothing acknowledges it. That plate is the whole signal.\n\n" +
                "The fragments are all sentences from act six said years early — \"I got as far as the " +
                "door\", \"I was not enough\". Strange the first time, and exact after the flashback.",
                MessageType.Warning);

            using (new EditorGUILayout.HorizontalScope())
            {
                EditorGUILayout.LabelField("Add the tell above this line:", GUILayout.Width(190f));

                if (GUILayout.Button(new GUIContent("Yua, aged nine"), EditorStyles.miniButton))
                {
                    InsertChildVoiceAbove(Speaker.YuaChild);
                }

                if (GUILayout.Button(new GUIContent("Haru, aged nine"), EditorStyles.miniButton))
                {
                    InsertChildVoiceAbove(Speaker.HaruChild);
                }
        ...
        /// <summary>
        /// The child speaker of a line shortly above this one, or
        /// <see cref="Speaker.Narrator"/> for none.
        /// </summary>
        /// <remarks>
        /// Three beats of slack, because the idiom is child line, hold, silence
        /// — and somebody may reasonably put a sound cue in there too.
        /// </remarks>
        private Speaker ChildVoiceAbove(int index)
        {
            SerializedProperty beats = _serialized.FindProperty("Beats");
            ...
            for (int i = index - 1; i >= 0 && i >= index - 4; i--)
            {
                ...
                if ((StoryBeatKind)beat.FindPropertyRelative("Kind").intValue != StoryBeatKind.Line)
                {
                    continue;
                }
                ...
                if (CharacterArt.IsChild(speaker))
                {
                    return speaker;
                }
            }
        ...
        /// <summary>Writes the missing half of a silence, above the ticked one.</summary>
        private void InsertChildVoiceAbove(Speaker child)
        {
            SerializedProperty beats = _serialized.FindProperty("Beats");
            int at = Mathf.Clamp(_list.index, 0, beats.arraySize);

            // The hold first, so that after both inserts the order reads
            // child line, hold, silence.
            beats.InsertArrayElementAtIndex(at);
            SerializedProperty hold = beats.GetArrayElementAtIndex(at);
            ResetBeat(hold, StoryBeatKind.Beat);
            hold.FindPropertyRelative("Seconds").floatValue = 2f;

            beats.InsertArrayElementAtIndex(at);
            SerializedProperty line = beats.GetArrayElementAtIndex(at);
            ResetBeat(line, StoryBeatKind.Line);
            line.FindPropertyRelative("Speaker").intValue = (int)child;
            line.FindPropertyRelative("Portrait").intValue = (int)Portrait.Unchanged;
            line.FindPropertyRelative("MeasurePatience").boolValue = false;
            ...
            Debug.Log(
                $"[Story] Added a line for {child} above the silence. Write one short fragment in it — " +
                "something that only makes sense after act six — and leave the face unchanged: the child " +
                "is in the name plate and the voice, not in the sprite.");
        }
        ...
        private void DrawChoiceBeat(SerializedProperty beat)
        {
            ...
            for (int i = 0; i < choices.arraySize; i++)
            {
                ...
                using (new EditorGUILayout.VerticalScope(EditorStyles.helpBox))
                {
                    ...
                    EditorGUILayout.PropertyField(
                        choice.FindPropertyRelative("BranchLength"),
                        new GUIContent(
                            "Lines behind it",
                            "How many of the beats after this choice belong to this option. 0 means the " +
                            "choice changes nothing. Options' blocks sit end to end in the order listed, " +
                            "and may hold spoken lines only."));
                }
            }
            ...
            EditorGUILayout.HelpBox(
                ...
                "'Lines behind it' is how many beats after this choice belong to each option. Leave it " +
                "at 0 and the choice changes nothing; set it and the two roads run one after another " +
                "here in the list, with only the chosen one played. Roads are easiest to edit from the " +
                "act's builder — hand-editing the numbers here has to keep them matching the beats.",
                MessageType.None);
        ...
        private void DrawSoundBlock(SerializedProperty beat, bool required)
        {
            ...
            using (new EditorGUILayout.HorizontalScope())
            {
                ...
                {
                    PreviewSound((SfxId)sound.intValue);
                }
                ...
                if (GUILayout.Button(
                        ...
                        GUILayout.Width(56f)))
                {
                    SfxWavExporter.ExportAndReveal((SfxId)sound.intValue);
                }
        ...
        private List<Finding> Check()
        {
            ...
            for (int i = 0; i < _act.Count; i++)
            {
                ...
                switch (beat.Kind)
                {
                    case StoryBeatKind.Line:
                        ...
                        if (CharacterArt.IsChild(beat.Speaker))
                        {
                            childHints++;
                        }
                    ...
                    case StoryBeatKind.Choice:
                        ...
                        // Each option may own a run of beats after the choice.
                        // Two things can go wrong with them and neither shows up
                        // at run time: a block that runs off the end of the act,
                        // and a block holding anything but a spoken line, which
                        // would be replayed by the save system whether the road
                        // was taken or not.
                        int branchTotal = 0;
                        if (beat.Choices != null)
                        {
                            ...
                            {
                                branchTotal += Mathf.Max(0, beat.Choices[c].BranchLength);
                            }
                        }

                        if (branchTotal > 0)
                        {
                            if (i + branchTotal >= _act.Count)
                            {
                                findings.Add(new Finding(
                                    MessageType.Error,
                                    $"Beat {i:000} is a choice whose options claim {branchTotal} beats after it, " +
                                    $"and the act ends before that. Rebuild the act from its builder.",
                                    i));
                            }
                            else
                            {
                                for (int b = i + 1; b <= i + branchTotal && b < _act.Count; b++)
                                {
                                    BeatData inside = _act.At(b);

                                    if (inside != null && inside.Kind != StoryBeatKind.Line)
                                    {
                                        findings.Add(new Finding(
                                            MessageType.Error,
                                            $"Beat {b:000} sits inside one road of the choice at beat {i:000} and " +
                                            $"is a {inside.Kind}, not a line. A road may hold spoken lines only — " +
                                            "move this past the point where the roads rejoin.",
                                            b));
                                    }
                                }
                            }
            ...
            if (asideDepth > 0)
            {
                findings.Add(new Finding(
                    MessageType.Error,
                    "This act turns to the player and never turns back. Every scene after that point is " +
                    "played in a dimmed room with the slow typing on. Add an Exit Aside."));
            }

            if (measuredSilences > 0 && childHints == 0 && _act.ActNumber > 0 && _act.ActNumber < 6)
            {
                findings.Add(new Finding(
                    MessageType.Info,
                    "This act has a silence in it and nothing in the nine-year-old's voice anywhere. The " +
                    "child name plate a beat before a silence is the only sign the game gives that one is " +
                    "worth staying at — without it the five minutes are unguessable."));
            }
        ...
        private void DrawGuideTab()
        {
            ...
            DrawPersianGuideLine(
                "سکوت دو بیت است نه یکی. تیک One of the silences را که بزنی، پنج دقیقه دست نزدن بازیکن یک بار برای پایان‌ها شمرده میشود — ولی به تنهایی پیدا نشدنی است. یکی دو بیت بالاتر باید یکی با صدای کودکیش حرف بزند تا پلاک اسم «کودک» را نشان بدهد. همان بیتِ تیک‌خورده خودش دکمه‌ی ساختنِ نیمه‌ی دوم را به تو میدهد.");

            DrawPersianGuideLine(
                "چهار صحنه در بازی مستقیم با بازیکن حرف میزنند. دورشان Enter Aside و Exit Aside بگذار: اتاق پشت گوینده تاریک میشود، متن کندتر تایپ میشود و باکس روی زمینه‌ی سنگین‌تری مینشیند. باز گذاشتنش خطا است — هر صحنه‌ی بعدش در تاریکی اجرا میشود.");

            DrawPersianGuideLine(
                "هر چیزی که مالِ یک پرده است داخل همین پنجره تنظیم میشود. چیزهایی که مالِ کلِ بازی هستند — شرط پایان‌ها، طول پنج دقیقه، متن‌های رابط کاربری — داخل کد هستند. فهرست کاملش بالای همین تب با نقطه‌ی پر و خالی مشخص شده.");
        ...
        /// <summary>
        /// Every mechanism in the game, and the one place each is set.
        /// </summary>
        /// <remarks>
        /// <para>
        /// Written because "is it all in the Story Editor?" is a fair question
        /// with an honest answer of "nearly", and the difference matters when
        /// somebody is about to write the whole game. Anything that belongs to
        /// one act is here. Anything that is a fact about the game — how long
        /// five minutes is, what earns an ending, what the interface says in
        /// three languages — is not, and pretending otherwise would send
        /// somebody looking through these tabs for a control that does not
        /// exist.
        /// </para>
        /// <para>
        /// Kept next to the thing it describes rather than in a document,
        /// because a document about a window is read once and this is read every
        /// time somebody cannot find something.
        /// </para>
        /// </remarks>
        private void DrawMechanismMap()
        {
            DrawMechanism("Dialogue, narration, all three languages", "Script tab · Line beats", true);
            DrawMechanism("Who is on stage and what face they wear", "Script tab · Enter / Exit / Clear Stage", true);
            DrawMechanism("Where the scene is", "Script tab · Background beats", true);
            DrawMechanism("Blue and green choices, and Yua overruling blue", "Script tab · Choice beats", true);
            DrawMechanism("The random interludes (the leg, the machine room)", "Script tab · Interlude beats", true);
            DrawMechanism("A silence, and the child's line that signals it", "Script tab · the tick, and its button", true);
            DrawMechanism("Turning to the player", "Script tab · Enter Aside / Exit Aside", true);
            DrawMechanism("Act five's frame, veil, box and blood", "Script tab · Open Frame, Grade, Pull Down, Stain", true);
            DrawMechanism("Act six playing itself", "Script tab · Enter Cinema", true);
            DrawMechanism("Act seven's film", "Script tab · Video beat, and the file name in it", true);
            DrawMechanism("Ending the game and wiping the saves", "Script tab · End Game beat", true);
            DrawMechanism("Music per act, and mid-act changes", "Act header · Music, and Music beats", true);
            DrawMechanism("Recorded lines", "Script tab · the Recording field on a Line", true);
            DrawMechanism("Typing speed for one line", "Script tab · Typing speed on a Line", true);
            DrawMechanism("Where the characters stand and how big", "Stage tab", true);
            DrawMechanism("How strong the veil is, and the frame", "Stage tab", true);

            EditorGUILayout.Space(4f);

            DrawMechanism("What earns each ending", "Code · Narrative/Endings.cs", false);
            DrawMechanism("How long five minutes is", "Code · Core/GameConfig.cs — and the Endings menu, to test", false);
            DrawMechanism("Interface text in three languages", "Code · Localization/LocalizationDatabase.cs", false);
            DrawMechanism("Save slots, pause menu, the title screen", "Code · UI/ and Scenes/", false);
            DrawMechanism("Every sound effect (all synthesised)", "Code · Audio/ProceduralSfxLibrary.cs", false);
            DrawMechanism("Building the whole game in one press", "Menu · Prepare The Whole Game", false);
            DrawMechanism("What is still missing", "Menu · Is The Game Ready?", false);
            DrawMechanism("Reaching an ending on purpose", "Menu · Endings ▸ …", false);
        }

        private static void DrawMechanism(string mechanism, string where, bool inThisWindow)
        {
            using (new EditorGUILayout.HorizontalScope())
            {
                GUILayout.Label(inThisWindow ? "●" : "○", GUILayout.Width(14f));
                EditorGUILayout.LabelField(mechanism, GUILayout.Width(300f));
                EditorGUILayout.LabelField(where, EditorStyles.miniLabel);
            }
        }
        ...
        {
            Speaker speaker = (Speaker)beat.FindPropertyRelative("Speaker").intValue;
            Portrait portrait = (Portrait)beat.FindPropertyRelative("Portrait").intValue;
